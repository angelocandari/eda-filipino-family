...
```{r Income_distribution, echo=FALSE, message=FALSE, warning=FALSE}
library(ggplot2)
...
```{r Income_log10, echo=FALSE, message=FALSE, warning=FALSE}
library(scales)
set.seed(222)
...
```{r GroupBy_workclass,echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Groupby_CustomClass, message=FALSE, warning=FALSE}
library(stringr)
library(dplyr)
...
```{r Reveiw_workerClass, message=FALSE, warning=FALSE}
fies.workers <- fies %>%
...
```{r Finding_waiterOutlier, message=FALSE, warning=FALSE}
...
```{r review_farmer, message=FALSE, warning=FALSE}
fies.farm <- fies %>%
...
fies.w.exp <- fies %>%
  ...
  mutate(exp_food = exp_bread + exp_rice + exp_meat + 
         ...
         exp_lux = exp_resto_hotel + exp_spec + exp_comms + exp_clothe,
         exp_vice = exp_alcoh + exp_taba,
         exp_util = exp_house_water + exp_rent) %>%
  gather(exp_type, exp_v, c(exp_food, exp_lux, exp_vice, exp_util,
                            exp_med,
                            exp_trans,
                            exp_edu,
                            exp_farm,
...
```{r}
ggplot(fies.w.exp, aes(income, expense, color = exp_type)) +
  geom_point(alpha = .2) +
  scale_x_continuous(limits = c(0, quantile(fies$income, .98))) +
  scale_y_continuous(limits = c(0, quantile(fies$expense, .98))) +
  facet_wrap(~head_class)
