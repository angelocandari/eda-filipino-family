...
There are lot of low-income and few high-income households that would make the graph skew to the left.
...
I wanted to see where this average exist in our distribution. Since the distribution is skewed by the large disparity in income, we transform the a histogram by log base 10. Think of it like we bring the incomes bins closer together to get a better view on the distribution.
...
## Looking at the Average with the Custom Working Class
...
Out of the total 41,544 households, 23,897 have 2 sources of income. We extract these observations and mold the data to enable us to see the where these income are coming from according to our work class.

```{r}
ggplot(fies.dual, aes(head_class, prop, color = inc_type)) +
  geom_boxplot()
```
